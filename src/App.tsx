...
const App = () => {  
    ...
    const checkWallet = async () => {  
      ...
      if (hasProvider) {  
        try {  
          ...
          setConnectedAddress(accounts[0]?.toLowerCase() || null);  
          setErrorMessage(null); // Clear error message on success  
        } catch (error) {  
          console.error("Error getting accounts:", error);  
          setConnectedAddress(null);  
          setErrorMessage("Failed to retrieve account information. Please try again.");  
        }  
      } else {  
        setErrorMessage("Ronin Wallet is not installed or ready.");  
      }  
      setLoading(false);  
    };  
  ...
  const isAdmin = connectedAddress === ADMIN_ADDRESS.toLowerCase();  
              ...
              <div className="flex items-center justify-between h-16">  
                <div className="flex items-center space-x-8">  
                  ...
                  <div className="hidden md:flex space-x-4">  
                    <a  
                      href="#/"  
                      className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${currentPage === "home" ? "bg-gray-800 text-white" : "text-gray-300 hover:bg-gray-700"}`}  
                    >  
                      Home  
                    </a>  
                    <a  
                      href="#/staking"  
                      className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${currentPage === "staking" ? "bg-gray-800 text-white" : "text-gray-300 hover:bg-gray-700"}`}  
                    >  
                      Staking  
                    </a>  
                    <a  
                      href="#/delegators"  
                      className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${currentPage === "delegators" ? "bg-gray-800 text-white" : "text-gray-300 hover:bg-gray-700"}`}  
                    >  
                      Delegators  
                    </a>  
                    {isAdmin && (  
                      <a  
                        href="#/admin"  
                        className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${currentPage === "admin" ? "bg-gray-800 text-white" : "text-gray-300 hover:bg-gray-700"}`}  
                      >  
                        Admin  
                      </a>  
                    )}  
